...
**Vendored fork of [tgrep](https://github.com/microsoft/tgrep) (MIT, Copyright
(c) Microsoft Corporation)** — the upstream project is what powers fast grep
searches in [GitHub Copilot CLI](https://github.com/github/copilot-cli).
See `NOTICE` for the attribution.

## Why?

Tools like `grep` and `ripgrep` scan every file on every search — O(total bytes)
per query. In a 100k+ file monorepo, that's painfully slow. kolargrep pre-builds a
trigram index so searches only touch the small set of files that could match.

**Start a server once, search instantly forever.**

```bash
kolargrep index .            # build the trigram index
kolargrep serve .            # start server (watches for file changes)
kolargrep "fn main" .        # instant — auto-connects to running server
```

Using kolargrep from an AI coding agent? See [AGENTS.md](AGENTS.md).
...
| Repo | Files | Platform | ripgrep | kolargrep | Speedup |
| --- | ---: | --- | ---: | ---: | ---: |
...
kolargrep wins 17 of the 18 measured cells; the exception is Kubernetes on Linux, a
...
```
kolargrep <pattern> ---TCP---> kolargrep serve (multi-client)
    (client)                   |
                          HybridIndex
                          /         \
...
kolargrep is designed to be significantly faster than ripgrep on large repos:
...
```bash
kolargrep index .                          # index current directory
kolargrep index /path/to/repo             # index a specific repo
kolargrep index . --index-path /tmp/idx   # custom index location
kolargrep index . --exclude vendor --exclude third_party  # skip directories
```
...
being indexed rather than kolargrep's own use. Indexing a single 2 GiB file holds
...
kolargrep says so rather than leaving you to guess:
...
```bash
kolargrep index . --no-require-git
kolargrep serve --no-require-git
```
...
kolargrep reads `core.ignorecase` and matches the way the repository itself does.
...
`--no-ignore`, `--max-filesize`, `--exclude` — must match between the `kolargrep
index` that built an index and the `kolargrep serve` that serves it.
...
```bash
kolargrep index . --max-filesize 8M
kolargrep serve   --max-filesize 8M
```
...
```bash
kolargrep index .                                 # external, 64 MB arena
kolargrep index . --index-buffer 16               # smaller arena, lower peak
kolargrep index . --index-strategy=memory         # opt out: sort entirely in RAM
```
...
Two caveats on reading those numbers. The peak kolargrep prints is *private bytes*,
...
`kolargrep serve` uses the same bounded builder when it has to create an index from
scratch, so starting a server on an unindexed repo costs the same memory as
`kolargrep index` (**148.6 MiB** rather than 1.6 GiB on the Linux kernel, and 2.6x
...
```bash
kolargrep serve .                          # start server (auto-builds index if missing)
kolargrep serve . --index-path /tmp/idx    # custom index location
kolargrep serve . --watch-mode poll        # poll without any native subscriptions
kolargrep serve . --poll-interval 60       # polling cadence after fallback
kolargrep serve . --watch-budget 4096      # lower this process's native watch ceiling
kolargrep serve . --no-watch              # disable all automatic refresh
kolargrep serve . --exclude node_modules   # exclude directories from indexing
```
...
`kolargrep status` reports indexing progress and hidden-file coverage. Legacy
...
`kolargrep serve` and `kolargrep index`:
...
Automatic refresh has two modes, configured on `kolargrep serve`:
...
native subscriptions**, including on Linux. It uses kolargrep's metadata
...
kolargrep does not probe the configured Linux quota: it uses this fixed ceiling
...
(including Linux directory-open events from kolargrep's own walks) are discarded
...
On Linux and Android, kolargrep registers only the non-ignored directories with
...
```bash
kolargrep "pattern" .                 # basic regex search
kolargrep "pattern" file1.rs file2.rs # search multiple files/paths
kolargrep "TODO|FIXME" .              # alternations
kolargrep '\w+(?!_test)' .            # PCRE-style lookahead fallback
kolargrep "error" . -i                # case-insensitive
kolargrep "error" . -S                # smart-case (auto if all lowercase)
kolargrep -F "Vec<T>" .               # literal string
kolargrep "MyStruct" . -l             # filenames only
kolargrep "pattern" . -c              # count per file
kolargrep "pattern" . -o              # only matching text
kolargrep "pattern" . -w              # whole word
kolargrep "pattern" . -v              # invert match
kolargrep "pattern" . -m 5            # max 5 matches per file
kolargrep "pattern" . -g "*.rs"       # glob filter
kolargrep "pattern" . -g "*.rs" -g "*.toml"  # multiple globs (OR)
kolargrep "pattern" . -t rust         # type filter
kolargrep "pattern" . -e "also_this"  # multiple patterns
kolargrep "pattern" . -A 3            # 3 lines after match
kolargrep "pattern" . -B 2            # 2 lines before match
kolargrep "pattern" . -C 3            # 3 lines before & after
kolargrep "pattern" . --json          # ripgrep-compatible JSON stream
kolargrep "pattern" . --vimgrep       # vim-compatible output
kolargrep "pattern" . --stats         # show query plan & timing
kolargrep "pattern" . --no-index      # brute-force (skip index)
kolargrep "pattern" . -U              # multiline matching
kolargrep "pattern" . -q              # quiet: exit code only
kolargrep "pattern" . --files-without-match  # files that DON'T match
kolargrep "pattern" . --no-filename   # suppress filenames
kolargrep "pattern" . -N              # suppress line numbers
kolargrep --files .                   # list searchable files
kolargrep --files src/main.rs         # list a single file if searchable
kolargrep --files -t rust .           # list Rust files only
kolargrep --type-list                 # show all file types
```
...
```bash
kolargrep count-files .              # count text files (no server needed)
kolargrep count-files /path/to/repo  # scan a specific repo
```
...
By default kolargrep sniffs a UTF-8/UTF-16LE/UTF-16BE BOM and decodes accordingly,
...
`--mmap`/`--no-mmap` (kolargrep always reads files directly), `--crlf`/`--no-crlf`
(a trailing `\r` is always stripped), `--no-config` (kolargrep reads no config
...
```bash
kolargrep -e needle .            # searches for "needle" under .
kolargrep -e needle -e other .   # both patterns, still just one path
```

### Output defaults

kolargrep matches ripgrep's context-dependent defaults rather than fixed ones:

- **Line numbers** are on only when stdout is a terminal. Piping to another
  command drops them, so `kolargrep needle . | cut -d: -f1` behaves the same as it
...
- **Paths** are printed by appending onto the argument you typed: the argument
  ...
  `kolargrep needle src/` prints `src/main.rs` while `kolargrep needle src` prints
...
of its matches are still reported, so `kolargrep -m1 --vimgrep foo` prints one row
...
`--stats` `bytes_searched` is lower than kolargrep's, which searches from a
...
kolargrep reports the real match position instead.
...
kolargrep additionally rejects ~65 binary file *extensions* during the walk to keep
...
`kolargrep index` to rebuild, or start a current `kolargrep serve` to reconcile and
...
ripgrep searches raw bytes. kolargrep decodes first, repairing any undecodable byte
...
- A pattern can match the substituted `U+FFFD`; ripgrep, seeing raw bytes, never
  matches there. So `kolargrep '.' ` finds one more match per repaired byte.
...
The divergence is affordable because kolargrep is not a one-shot scanner. A file a
...
- A file named directly on the command line is never dropped by the *inherited*
  default, only by a limit you passed. `kolargrep pattern ./huge.log` searches
  `huge.log`.
...
kolargrep reports it on stderr, skips the offending rule and carries on, leaving the
...
3. **Serving** — `kolargrep serve` wraps the index in a HybridIndex, watches for
...
└── kolargrep-cli/                # Binary crate
    └── src/
        ├── main.rs               # CLI entry (clap)
        ├── index.rs              # `kolargrep index`
        ├── search.rs             # `kolargrep <pattern>` with server delegation
        ├── serve.rs              # `kolargrep serve` (TCP JSON-RPC + file watcher)
        ├── status.rs             # `kolargrep status`
...
cd kolargrep
cargo install --path kolargrep-cli --locked
```

### Homebrew (Linux, macOS)

```bash
brew install kolargrep
```
...
tar xzf "$tmpdir"/kolargrep-*-x86_64-unknown-linux-musl.tar.gz -C "$tmpdir"
install -Dm755 "$tmpdir/kolargrep" "$HOME/.local/bin/kolargrep"
rm -rf "$tmpdir"

# macOS (Apple Silicon)
gh release download --repo microsoft/tgrep -p '*aarch64-apple-darwin*' -D /tmp/kolargrep-dl
tar xzf /tmp/kolargrep-dl/kolargrep-*-aarch64-apple-darwin.tar.gz -C /usr/local/bin

# macOS (Intel)
gh release download --repo microsoft/tgrep -p '*x86_64-apple-darwin*' -D /tmp/kolargrep-dl
tar xzf /tmp/kolargrep-dl/kolargrep-*-x86_64-apple-darwin.tar.gz -C /usr/local/bin
```

```powershell
# Windows (PowerShell)
gh release download --repo microsoft/tgrep -p '*windows*' -D $env:TEMP\kolargrep-dl
Expand-Archive $env:TEMP\kolargrep-dl\kolargrep-*-windows*.zip -DestinationPath $HOME\.cargo\bin -Force
```
