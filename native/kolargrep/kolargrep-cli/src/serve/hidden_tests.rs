...
fn legacy_hidden_coverage_is_published_only_after_reconciliation() {
    ...
    *state.stale_refresh_hook.lock().unwrap() = Some(Arc::new(move |phase| {
        if matches!(phase, StaleRefreshPhase::AfterMatcherPublish) {
            ...
            assert!(
                !kolargrep_core::meta::IndexMeta::load(&state.index_dir)
                    .unwrap()
                    .hidden_complete
            );
    ...
    *state.stale_refresh_hook.lock().unwrap() = None;
    ...
    assert!(state.hidden_complete.load(Ordering::SeqCst));
    assert!(
        kolargrep_core::meta::IndexMeta::load(&index_dir)
            .unwrap()
            .hidden_complete
    );
...
fn reload_preserves_hidden_coverage_visibility_and_storage_exclusion() {
    ...
    for _ in 0..2 {
        ...
        let response: serde_json::Value = serde_json::from_str(&response).unwrap();
        assert!(response.get("error").is_none(), "{response}");
        let meta = kolargrep_core::meta::IndexMeta::load(&index_dir).unwrap();
...
fn hidden_filename_publication_is_coherent_even_when_metadata_publish_fails() {
    ...
    *state.stale_refresh_hook.lock().unwrap() = Some(Arc::new(move |phase| {
        if matches!(phase, StaleRefreshPhase::AfterFilenameSidecarPublish) {
            ...
            let sidecar = kolargrep_core::path_index::read_filename_index(&state.index_dir)
                .unwrap()
                .unwrap();
            ...
            let old_meta = kolargrep_core::meta::IndexMeta::load(&state.index_dir).unwrap();
    ...
    let reader = kolargrep_core::reader::IndexReader::open(&index_dir).unwrap();
    let old_meta = kolargrep_core::meta::IndexMeta::load(&index_dir).unwrap();
    let restarted =
...
fn filename_only_legacy_upgrade_publishes_the_core_format_boundary() {
    ...
    std::fs::write(root.join("visible.png"), "needle\n").unwrap();
    std::fs::write(root.join(".secret.png"), "needle\n").unwrap();
    let mut meta = kolargrep_core::meta::IndexMeta::new(&root.to_string_lossy(), 0, 0);
    meta.version = 2;
    ...
    let meta = kolargrep_core::meta::IndexMeta::load(&index_dir).unwrap();
    assert_eq!(meta.version, kolargrep_core::meta::INDEX_FORMAT_VERSION);
