...
use kolargrep_core::builder::{self, BuildOptions, IndexStrategy};
...
/// Everything `kolargrep index` needs to build an index.
///
...
pub fn run(opts: RunOptions<'_>) -> Result<()> {
    ...
    // Peak is an OS high-water mark for the whole process. `kolargrep index` does
