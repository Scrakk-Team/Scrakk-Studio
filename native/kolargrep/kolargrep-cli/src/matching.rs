...
pub enum SearchMatcher {
    ...
    /// compile (see `kolargrep_core::query::relax_for_indexing`). Relaxation only
...
fn build_fancy_prefilter(combined: &str, wrap: &dyn Fn(&str) -> String) -> Option<regex::Regex> {
    let relaxed = kolargrep_core::query::relax_for_indexing(combined)?;
    if relaxed == combined {
        return None;
    }
...
mod tests {
    ...
    fn relaxation_is_a_superset_of_every_pattern_it_accepts() {
        ...
        for pattern in patterns {
            ...
            let Some(relaxed_src) = kolargrep_core::query::relax_for_indexing(pattern) else {
