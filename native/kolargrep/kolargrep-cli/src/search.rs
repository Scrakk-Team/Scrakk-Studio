/// `kolargrep search` — search using the trigram index, with server delegation.
///
...
use kolargrep_core::filetypes;
use kolargrep_core::meta::IndexMeta;
use kolargrep_core::query::{self, QueryPlan};
use kolargrep_core::reader::IndexReader;
...
pub struct SearchOptions {
    ...
    /// [`kolargrep_core::walker::DEFAULT_MAX_FILE_SIZE`]. See
    /// [`exceeds_max_filesize`].
    pub max_filesize_requested: bool,
    pub encoding: kolargrep_core::encoding::EncodingMode,
    pub follow: bool,
...
fn load_indexed_file_paths(
    index_dir: &Path,
) -> Result<Option<(Vec<String>, kolargrep_core::visibility::PathVisibility)>> {
    if !index_dir.join("lookup.bin").exists() {
        return Ok(None);
    }
    let filename_index = match kolargrep_core::path_index::read_filename_index(index_dir) {
        Ok(Some(index)) => index,
...
fn write_indexed_file_paths(
    ...
    visibility: Option<&kolargrep_core::visibility::PathVisibility>,
...
fn warn_missing_index(index_dir: &Path, explicit_path: bool, quiet: bool) {
    ...
    if explicit_path {
        eprintln!("note: build one with `kolargrep index --index-path {dir}`");
    } else {
        eprintln!(
            "note: if a server is running with `--index-path <dir>`, pass the same \
             --index-path here; otherwise build an index with `kolargrep index`"
        );
    }
}
...
fn search_local_index(
        ...
        || meta.version != kolargrep_core::meta::INDEX_FORMAT_VERSION
    {
        if !opts.quiet && !opts.no_messages {
            eprintln!(
                "warning: index lacks complete hidden-file coverage - scanning filesystem; rebuild with `kolargrep index` or upgrade with `kolargrep serve`"
            );
        }
        return brute_force_search(root, index_dir, opts, ci, writer);
    }
    let reader = IndexReader::open(index_dir)?;
    let filename_visibility = match kolargrep_core::path_index::read_filename_index(index_dir) {
    ...
    // the two, `kolargrep --index-path IDX foo src` looks for `src/src/lib.rs` and
...
fn try_map_text(
    ...
    if !kolargrep_core::encoding::borrows_whole_input(&map, encoding) {
        return None;
    }
...
fn read_text_lossy(
    ...
    if let Some((text, first_nul)) = try_map_text(path, encoding) {
        return Ok(FileRead {
            text,
            fixups: kolargrep_core::encoding::LossyFixups::default(),
            first_nul,
        });
    }
...
/// from the *inherited* [`kolargrep_core::walker::DEFAULT_MAX_FILE_SIZE`], because
...
mod tests {
    ...
    fn display_path_strips_extended_length_prefix() {
        assert_eq!(
            display_path(Path::new(r"\\?\C:\src\repo\.kolargrep")),
            r"C:\src\repo\.kolargrep"
        );
    }
