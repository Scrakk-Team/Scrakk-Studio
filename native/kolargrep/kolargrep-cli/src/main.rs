/// kolargrep — trigram-indexed grep with client/server architecture.
///
/// Usage:
///   kolargrep index [path]           Build the trigram index
///   kolargrep serve [path]           Start the search server
///   kolargrep <pattern> [path]       Search (auto-delegates to server)
///   kolargrep status [path]          Show index/server status
mod cpu;
mod glob_filter;
mod index;
mod matching;
...
struct Cli {
    ...
    /// Number of threads to use. kolargrep sizes its pool automatically.
    #[arg(short = 'j', long = "threads", value_name = "NUM", global = true)]
    threads: Option<usize>,

    /// Accepted for compatibility; kolargrep always reads files directly.
    ...
    /// Accepted for compatibility; kolargrep reads no configuration file.
    #[arg(long = "no-config", global = true)]
    no_config: bool,

    /// Accepted for compatibility; kolargrep's colors are not configurable yet.
    ...
    /// Accepted for compatibility; kolargrep always strips a trailing `\r`.
    ...
    /// Not supported: kolargrep does not decompress archives before searching.
...
struct ResolvedArgs {
    ...
    /// [`kolargrep_core::walker::DEFAULT_MAX_FILE_SIZE`]. A file named directly on
...
/// Posting accumulation strategy for `kolargrep index`.
...
impl Cli {
    ...
    fn max_filesize_bytes(&self) -> Result<Option<u64>> {
        ...
        match self.max_filesize.as_deref() {
            ...
            None => Ok(kolargrep_core::walker::DEFAULT_MAX_FILE_SIZE),
        }
    }
    ...
    /// does.
    fn encoding_mode(&self) -> Result<kolargrep_core::encoding::EncodingMode> {
        match self.encoding.as_deref() {
            Some(label) => kolargrep_core::encoding::parse_encoding(label),
            None => Ok(kolargrep_core::encoding::EncodingMode::Auto),
        }
    }
    ...
    /// `kolargrep search` over the same tree. That is indistinguishable from a
...
fn reject_unsupported_discovery_flags(cli: &Cli, subcommand: &str) {
    ...
    eprintln!(
        "kolargrep: `{subcommand}` does not support {}; \
...
fn main() {
    ...
    // and kolargrep declares enough flags that an unoptimised build overflows the
...
fn run_cli() {
    ...
    if cli.search_zip {
        eprintln!("kolargrep: -z/--search-zip is not supported; decompress the files first");
        process::exit(2);
    }

    // `--pcre2-version` reports the backtracking engine and exits, like ripgrep.
    // kolargrep links fancy-regex rather than PCRE2 itself, so say so instead of
    ...
    let result = match cli.command {
        ...
        None => {
            ...
            } else if cli.pattern.is_some() || !cli.regexp.is_empty() || cli.pattern_file.is_some()
            {
                ...
                eprintln!("Usage: kolargrep <pattern> [PATH ...]");
                eprintln!("       kolargrep index [path]");
                eprintln!("       kolargrep serve [path]");
                eprintln!("       kolargrep status [path]");
                eprintln!("Search defaults to the current directory when no path is provided.");
                eprintln!("Run `kolargrep --help` for full usage.");
                process::exit(2);
            }
        }
    };
...
fn report_missing_path(path: &std::path::Path, no_messages: bool) {
    ...
    if !no_messages {
        ...
        eprintln!("kolargrep: {path}: IO error for operation on {path}: {reason}");
    }
}
...
mod tests {
    ...
    fn parse(args: &[&str]) -> std::result::Result<Cli, clap::Error> {
        let args: Vec<String> = std::iter::once("kolargrep")
