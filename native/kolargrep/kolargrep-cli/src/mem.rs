...
//! total physical memory — used to enforce an indexing memory budget so kolargrep
...
//!   doing anything — they are not memory kolargrep is holding.
...
//!   memory does kolargrep use" means, and it is what can actually exhaust a host.
//!
...
///
/// Prefers private bytes. The cap exists to stop kolargrep exhausting the host, and
...
/// exposes nothing.
///
/// Leads with private bytes, because that is the memory kolargrep is responsible
...
/// into a single number that would read as kolargrep's own use.
...
mod tests {
    ...
    // The cap exists to stop kolargrep exhausting the host, so it must charge
    ...
    fn peak_memory_report_names_the_working_set_only_when_it_is_much_larger() {
        ...
        // `kolargrep index` over three files measured 1.5 MiB private against a
