...
//! **complete** index.
//!
//! Starts `kolargrep serve` with a tiny `--max-memory` budget so the bulk indexer
...
/// Start `kolargrep serve` with a 1 MB memory cap so the indexer flushes mid-build.
fn start_capped_server(root: &Path) -> ServerGuard {
    let index_dir = root.join(".kolargrep_memcap_index");
    ...
    let port = loop {
        assert!(
            ...
            "kolargrep serve did not start within 30s"
        );
