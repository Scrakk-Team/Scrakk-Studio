...
fn stats_follow_matches_on_a_combined_output_stream() {
    ...
    let merged_path = dir.path().join("merged-output.txt");
    {
        ...
        let status = ProcessCommand::new(env!("CARGO_BIN_EXE_kolargrep"))
            .args([
                "--no-index",
                "--stats",
                "--no-heading",
...
fn serve_rejects_second_server_on_same_index() {
    ...
    let kolargrep_bin = assert_cmd::cargo::cargo_bin("kolargrep");
    let mut server1 = std::process::Command::new(&kolargrep_bin)
        .args([
            "serve",
            root.to_str().unwrap(),
            "--index-path",
    ...
    kolargrep()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "another kolargrep server is already running",
        ));
...
fn serve_rebuilds_when_existing_index_is_corrupted() {
    ...
    let kolargrep_bin = assert_cmd::cargo::cargo_bin("kolargrep");
    let mut server = std::process::Command::new(&kolargrep_bin)
        .args([
            "serve",
            root.to_str().unwrap(),
            "--index-path",
...
fn start_passthru_server(root: &std::path::Path, index_dir: &std::path::Path) -> PassthruServer {
    let mut child = std::process::Command::new(assert_cmd::cargo::cargo_bin("kolargrep"))
        .args([
            "serve",
            root.to_str().unwrap(),
            "--index-path",
        ...
        .spawn()
        .expect("failed to start kolargrep serve");

    let deadline = Instant::now() + Duration::from_secs(30);
    let serve_json = index_dir.join("serve.json");
    let port = loop {
        if let Some(status) = child.try_wait().unwrap() {
            panic!("kolargrep serve exited before becoming ready: {status}");
        }
        assert!(
            Instant::now() < deadline,
            "kolargrep serve did not become ready"
        );
        if let Ok(data) = fs::read_to_string(&serve_json)
            && let Ok(info) = serde_json::from_str::<serde_json::Value>(&data)
            && let Some(port) = info.get("port").and_then(|v| v.as_u64())
    ...
    loop {
        assert!(
            Instant::now() < deadline,
            "kolargrep serve did not finish indexing"
        );
        if let Ok(response) =
...
//
// These cover cases where kolargrep used to silently return no results, which is
...
fn indexed_multiple_patterns_match_the_same_as_brute_force() {
    ...
    let lines = |extra: &[&str]| -> Vec<String> {
        ...
        let out = kolargrep().args(&args).output().unwrap();
...
fn new_flags_agree_across_search_paths() {
    ...
    for case in cases {
        ...
        let local = kolargrep().args(&local_args).output().unwrap();

        let mut idx_args = vec!["--index-path", &idx];
        idx_args.extend_from_slice(case);
        idx_args.push(&path);
        let indexed = kolargrep().args(&idx_args).output().unwrap();
