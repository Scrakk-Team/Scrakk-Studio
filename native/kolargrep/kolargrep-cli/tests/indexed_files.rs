...
fn kolargrep_bin() -> PathBuf {
    assert_cmd::cargo::cargo_bin("kolargrep")
}
...
fn start_server(root: &Path, index: &Path) -> ServerGuard {
    let mut child = std::process::Command::new(kolargrep_bin())
        .args([
            "serve",
            "--no-watch",
            "--index-path",
...
fn indexed_files_include_non_content_paths_and_use_the_snapshot() {
    ...
    let indexed = output_lines(kolargrep().args([
        "--files",
        "--sort",
        "path",
        root.to_str().unwrap(),
    ...
    let walked = output_lines(kolargrep().args([
        "--files",
        "--no-index",
        root.to_str().unwrap(),
        "--index-path",
    ...
    let case_insensitive_ignore = output_lines(kolargrep().args([
        "--files",
...
fn legacy_filename_index_falls_back_and_indexed_scope_still_filters() {
    ...
    let scoped = output_lines(kolargrep().args([
        "--files",
        "--sort",
        "path",
        "-t",
    ...
    fs::remove_file(index.join(kolargrep_core::path_index::EXTRA_PATHS_FILENAME)).unwrap();
    fs::write(root.join("legacy-live.rs"), "fn legacy() {}\n").unwrap();
    let fallback = output_lines(kolargrep().args([
        "--files",
        root.to_str().unwrap(),
        "--index-path",
        index.to_str().unwrap(),
    ...
    for via_server in [false, true] {
        ...
        for (key, names) in [
            ...
            for flag in ["--sort", "--sortr"] {
                ...
                let listed = output_lines(kolargrep().args([
                    "--files",
                    "-t",
                    "rust",
                    flag,
...
fn files_uses_the_live_server_filename_index() {
    ...
    kolargrep_core::path_index::write_extra_paths(&index, &[]).unwrap();
    let listed = output_lines(kolargrep().args([
        "--files",
        root.to_str().unwrap(),
        "--index-path",
