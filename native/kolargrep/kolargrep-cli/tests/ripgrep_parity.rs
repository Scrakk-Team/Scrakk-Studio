//! Regression tests for divergences found by differential-testing kolargrep
//! against a real `ripgrep` binary (rg 15.2.0).
//!
//! Each block below corresponds to a behaviour that was empirically confirmed
//! by running the same arguments through `rg` and through `kolargrep`. The comment
...
fn kolargrep() -> Command {
    Command::cargo_bin("kolargrep").unwrap()
}
...
fn path_display_dot_argument_keeps_the_dot() {
    let dir = fixture();
    let out = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--no-heading",
        "needle",
        ".",
...
fn path_display_keeps_absolute_arguments_absolute() {
    ...
    let out = stdout_of(kolargrep().args(["--no-index", "--no-heading", "needle", &root]));
    assert!(
        out.contains(&root),
...
fn indexed_and_brute_force_agree_on_a_subtree() {
    ...
    let run = |extra: &[&str]| -> Vec<String> {
        ...
        let mut v: Vec<String> = stdout_of(kolargrep().current_dir(dir.path()).args(&args))
            .lines()
            .map(str::to_string)
            .collect();
        v.sort();
...
fn binary_flag_includes_extension_rejected_files() {
    let dir = fixture();
    // Text content behind a binary extension. kolargrep skips these by extension to
    // keep indexing cheap; --binary is the escape hatch back to rg's coverage.
    fs::write(dir.path().join("blob.dat"), "needle plain\n").unwrap();

    let without = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--no-heading",
        "needle",
        ".",
    ...
    let with = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--no-heading",
        "--binary",
        "needle",
...
fn svg_and_raw_are_classified_by_content_on_both_search_paths() {
    ...
    let run = |mode: &[&str]| -> String {
        ...
        stdout_of(kolargrep().current_dir(dir.path()).args(&args))
    };
    ...
    let typed = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--no-heading",
        "-t",
        "svg",
...
//
// kolargrep decodes invalid UTF-8 lossily, where each bad byte becomes a 3-byte
...
fn search_root_outside_the_index_falls_back_to_brute_force() {
    ...
    let out = stdout_of(kolargrep().args([
        "--no-heading",
        "--index-path",
        &idx,
        "needle",
...
fn max_columns_under_only_matching_measures_the_match_alone() {
    let dir = max_columns_fixture();
    let printed = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "-o",
        "-M",
        "10",
    ...
    let omitted = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "-o",
        "-M",
        "9",
...
fn unparseable_ignore_file_is_reported() {
    ...
    let err = stderr_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--ignore-file",
        "bad.ignore",
        "needle",
...
// `--stats` reports a smaller `bytes_searched` than kolargrep, which searches from
...
// line). kolargrep reports the real match position instead of reproducing that.
// ---------------------------------------------------------------------------
...
fn indexed_search_finds_matches_in_repaired_invalid_utf8() {
    ...
    let indexed = sorted_lines(&stdout_of(
        kolargrep().current_dir(dir.path()).args([pattern, "."]),
    ));
    let brute = sorted_lines(&stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        pattern,
        ".",
    ])));
...
fn assert_reports_os_error(relative: &str) {
    ...
    let stderr = stderr_of(kolargrep().current_dir(dir.path()).args(["needle", relative]));
...
fn per_file_json_elapsed_is_scoped_to_that_file() {
    ...
    let stdout = stdout_of(kolargrep().current_dir(dir.path()).args(["--json", "needle"]));
...
// kolargrep searches text in which invalid UTF-8 has been repaired to U+FFFD,
...
fn binary_note_offset_is_the_byte_in_the_file() {
    ...
    let out = stdout_of(kolargrep().args(["needle", path.to_str().unwrap()]));
...
fn json_binary_offset_and_bytes_searched_are_file_positions() {
    ...
    let stdout = stdout_of(kolargrep().args(["--json", "needle", path.to_str().unwrap()]));
    let end = stdout
        .lines()
        .filter_map(|l| serde_json::from_str::<serde_json::Value>(l).ok())
        .find(|m| m["type"] == "end")
...
fn json_ignores_only_matching() {
    let dir = json_stats_fixture();
    let with_o = stdout_of(kolargrep().current_dir(dir.path()).args([
        "--no-index",
        "--json",
        "-o",
        "alpha",
...
// file's context block to the next is a gap. kolargrep only marked gaps *within* a
