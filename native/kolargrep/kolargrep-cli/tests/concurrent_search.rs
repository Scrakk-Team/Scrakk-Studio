//! Integration tests for concurrent search requests to the kolargrep server.
//!
//! Starts a `kolargrep serve` instance and fires multiple parallel search requests
...
/// Returns the path to the kolargrep binary, located by Cargo.
fn kolargrep_bin() -> std::path::PathBuf {
...
/// Starts a kolargrep server and waits until it's ready to accept connections.
fn start_server(root: &Path) -> ServerGuard {
    let index_dir = root.join(".kolargrep_test_index");
    fs::create_dir_all(&index_dir).unwrap();

    let child = Command::new(kolargrep_bin())
        .args([
            "serve",
            "--no-watch",
            "--index-path",
    ...
    let port = loop {
        if start.elapsed() > Duration::from_secs(30) {
            panic!("kolargrep serve did not start within 30 seconds");
        }
