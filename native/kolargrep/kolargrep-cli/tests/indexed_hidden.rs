...
use kolargrep_core::path_index::{self, EXTRA_PATHS_FILENAME};
...
impl Fixture {
    ...
    fn build(&self, force: bool) {
        let mut command = Command::cargo_bin("kolargrep").unwrap();
        command
    ...
    fn query(
        ...
        command
            ...
            .output()
            .expect("failed to run kolargrep query")
    }
...
impl ServerGuard {
    fn start(fixture: &Fixture, corpus: &Corpus, mode: WatchMode, hidden: bool) -> Self {
        ...
        let mut command = std::process::Command::new(assert_cmd::cargo::cargo_bin("kolargrep"));
        command
...
fn hidden_local_status_requires_coherent_index_coverage() {
    ...
    for coverage in [
        ...
        let output = Command::cargo_bin("kolargrep")
            .unwrap()
            .timeout(WAIT_TIMEOUT)
            .arg("status")
            .arg(&fixture.root)
