...
fn wait_for_server(index_dir: &Path) -> (u32, u16) {
    ...
    loop {
        assert!(
            ...
            "kolargrep serve did not start within 60 seconds"
        );
...
fn assert_polling_server_refreshes(mode_args: &[&str]) {
    ...
    loop {
        let evidence = kolargrep_core::meta::read_file_evidence(&index_dir).unwrap();
...
fn watcher_does_not_subscribe_to_gitignored_directories() {
    ...
    let status = Command::new(kolargrep_bin())
        .args([
            "index",
            root.to_str().unwrap(),
            "--index-path",
        ...
        .status()
        .expect("failed to run kolargrep index");
