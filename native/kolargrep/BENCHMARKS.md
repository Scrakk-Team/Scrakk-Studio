...
kolargrep runs in client/server mode: `kolargrep serve` runs in the background, and the `kolargrep` client connects via TCP.

Every query is run through a **fresh `kolargrep` client process**, so each measurement includes process
...
Speedup = ripgrep total / kolargrep total across the whole query suite for that repo.
...
kolargrep more in delivery than the index saves in file selection. See
[What decides the margin](#what-decides-the-margin).

Shared-runner results are not controlled-machine measurements. Compare kolargrep with
...
land close to the old ones by coincidence; they measure the memory kolargrep owns
...
> These microbenchmark numbers come from a local `cargo bench -p kolargrep-core` run, not
...
```powershell
cargo bench -p kolargrep-core --bench trigram_extraction -- --sample-size 30 --warm-up-time 1 --measurement-time 2
cargo bench -p kolargrep-core --bench query_execution -- --sample-size 30 --warm-up-time 1 --measurement-time 2
cargo bench -p kolargrep-core --bench index_build -- --sample-size 10 --warm-up-time 0.5 --measurement-time 1
cargo bench -p kolargrep-core --bench index_build -- --peak-memory 5000
cargo bench -p kolargrep-core --bench index_build -- --peak-memory-high-diversity 1000
```
...
```powershell
cargo bench -p kolargrep-core --bench index_build -- --peak-memory-high-diversity 16000
```
...
alongside it. `kolargrep index` and `kolargrep serve` now lead with private bytes for
...
This is what the reported case was made of. A `kolargrep serve` opening an index
...
**What `peak memory` reports.** Mapping large files made the number kolargrep printed
...
the memory kolargrep held. A `kolargrep serve` bootstrap over a 290k-file internal
...
| --- | ---: | ---: | ---: |
| `kolargrep index` | 305.1 MiB | 318.0 MiB | 816 s |
| `kolargrep serve` bootstrap | 382.3 MiB | 386.8 MiB | 609 s |
...
being folded into a figure that reads as kolargrep's own use. The 2 GiB case now
...
`kolargrep serve` on an empty index used to build through a different path than
`kolargrep index`: it walked the repo and accumulated every posting in the live
...
A cold start now delegates to the same memory-bounded builder as `kolargrep index`.
...
**Smaller fixtures.** 2,500 files / 37.5 MiB built from kolargrep's own `.rs`
...
| kolargrep (client → serve) | 41,884 | 1,396.1 | — |

**kolargrep is ~17.6x faster**
...
| kolargrep (client → serve) | 79,293 | 2,643.1 | — |

**kolargrep is ~15.8x faster**
...
| kolargrep (client → serve) | 18,942 | 631.4 | — |

**kolargrep is ~3.81x faster**
...
| kolargrep (client → serve) | 56,435 | 462.6 | — |

**kolargrep is ~38.6x faster**
...
| kolargrep (client → serve) | 78,447 | 643.0 | — |

**kolargrep is ~51.9x faster**
...
| kolargrep (client → serve) | 19,815 | 162.4 | — |

**kolargrep is ~7.36x faster**
...
| kolargrep (client → serve) | 9,612 | 94.2 | — |

**kolargrep is ~34.8x faster**
...
| kolargrep (client → serve) | 26,124 | 256.1 | — |

**kolargrep is ~21.0x faster**
...
| kolargrep (client → serve) | 4,643 | 45.5 | — |

**kolargrep is ~9.38x faster**
...
| kolargrep (client → serve) | 19,759 | 193.7 | — |

**kolargrep is ~7.69x faster**
...
| kolargrep (client → serve) | 24,849 | 243.6 | — |

**kolargrep is ~2.69x faster**
...
| kolargrep (client → serve) | 9,115 | 89.4 | — |

**kolargrep is ~1.61x faster**
...
| kolargrep (client → serve) | 18,381 | 189.5 | — |

**kolargrep is ~7.08x faster**
...
| kolargrep (client → serve) | 9,881 | 101.9 | — |

**kolargrep is ~2.81x faster**
...
| kolargrep (client → serve) | 9,878 | 101.8 | — |

**ripgrep is ~1.08x faster** — kolargrep is 0.93x
...
| kolargrep (client → serve) | 8,093 | 78.6 | — |

**kolargrep is ~7.53x faster**
...
| kolargrep (client → serve) | 6,753 | 65.6 | — |

**kolargrep is ~3.12x faster**
...
| kolargrep (client → serve) | 3,516 | 34.1 | — |

**kolargrep is ~1.29x faster**
...
- **Repo size is the strongest predictor.** The trigram index eliminates scanning files
  ...
  (Chromium 504K files, gecko-dev 388K files) kolargrep wins on every platform, by 3.81–51.9x.
...
kolargrep loses exactly one of the eighteen cells — Kubernetes on Linux, at 0.93x — and
...
**Match volume**, which is easy to miss. kolargrep's cost per *delivered* match —
...
scans every file whichever pattern it gets — while kolargrep's fell from 49–128s on
...
Practical guidance: kolargrep is a strong replacement for ripgrep on large repos.
