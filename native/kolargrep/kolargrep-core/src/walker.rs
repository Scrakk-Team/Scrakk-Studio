...
/// divergence is affordable because kolargrep is not a one-shot scanner: a file
...
/// the failure mode pinned by `kolargrep-cli/tests/serve_max_filesize.rs`.
///
    ...
    walker.run(|| {
        ...
        Box::new(move |entry| {
            ...
            if let Some(err) = entry.error()
                && !no_ignore_messages
            {
                eprintln!("kolargrep: {}", display_ignore_error(err, walk_root));
            }
