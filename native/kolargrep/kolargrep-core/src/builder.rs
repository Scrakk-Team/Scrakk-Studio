...
const INDEX_DIR_NAME: &str = ".kolargrep";
...
///
/// This is used by `kolargrep serve` to build a bounded delta for files that are
...
/// far worse than it looks: for `kolargrep serve` this delta is how the index is
        ...
        let batch_data: Vec<std::result::Result<ExtractedFile, std::path::PathBuf>> = batch
            ...
            .map(|(path, &size)| {
                let data = match read_for_index(path, size, &owned_budget, None) {
                    Ok(data) => data,
                    Err(error) => {
                        eprintln!("kolargrep: skipping {}: {error}", path.display());
                ...
                if extracted.version.is_none() {
                    eprintln!(
                        "kolargrep: skipping {}: file changed during extraction or precise metadata is unavailable",
                        path.display()
                    );
                    return Err(path.clone());
                }
...
mod tests {
    ...
    /// An unreadable input must not sink the whole delta. For `kolargrep serve`
