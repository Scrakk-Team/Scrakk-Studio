...
use kolargrep_core::PostingEntry;
use kolargrep_core::query::{self, QueryPlan, TrigramQuery, execute_plan_with_masks};
...
fn create_common_literal_index(file_count: usize) -> (tempfile::TempDir, IndexReader, QueryPlan) {
    ...
    let index_dir = dir.path().join(".kolargrep_bench");
    kolargrep_core::builder::build_index(dir.path(), Some(&index_dir), false, false, &[]).unwrap();
