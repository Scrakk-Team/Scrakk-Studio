#!/usr/bin/env bash
#
# Benchmark kolargrep vs ripgrep on Linux/macOS.
#
# Usage:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --kolargrep-bin)   KOLARGREP_BIN="$2"; shift 2 ;;
    ...
    -h|--help)
      ...
      echo "  --bench-dir DIR    Working directory for artifacts (default: /tmp/kolargrep-bench)"
      echo "  --kolargrep-bin PATH   Path to kolargrep binary (default: target/release/kolargrep)"
      echo "  --results   PATH   Output path for results markdown"
      echo "  --skip-build       Skip building kolargrep from source"
      exit 0
      ;;
...
if [ -z "$KOLARGREP_BIN" ]; then
  KOLARGREP_BIN="$REPO_ROOT/target/release/kolargrep"
fi
if [ -z "$RESULTS_PATH" ]; then
  RESULTS_PATH="$BENCH_DIR/benchmark-results.md"
fi

INDEX_PATH="$BENCH_DIR/kolargrep-index"
...
if [ "$SKIP_BUILD" = false ]; then
  echo "==> Building kolargrep (release)..."
  (cd "$REPO_ROOT" && cargo build --release)
fi

if [ ! -x "$KOLARGREP_BIN" ]; then
  echo "error: kolargrep binary not found at $KOLARGREP_BIN" >&2
  echo "run 'make release' first or pass --kolargrep-bin" >&2
  exit 1
fi
...
# ── Build index ──
echo "==> Building kolargrep index..."
INDEX_START=$(now_ns)
"$KOLARGREP_BIN" index "$BENCH_REPO_DIR" --index-path "$INDEX_PATH"
INDEX_END=$(now_ns)
...
echo "==> Starting kolargrep serve..."

LOCKFILE="$INDEX_PATH/serve.json"
rm -f "$LOCKFILE"

"$KOLARGREP_BIN" serve "$BENCH_REPO_DIR" --index-path "$INDEX_PATH" --no-watch > /dev/null 2>&1 &
SERVE_PID=$!
...
echo "Waiting for kolargrep serve (pid $SERVE_PID)..."
READY=false
for i in $(seq 1 60); do
  if [ -f "$LOCKFILE" ]; then
    PORT=$(grep -o '"port":[0-9]*' "$LOCKFILE" | grep -o '[0-9]*')
    echo "kolargrep serve ready on port $PORT"
    READY=true
    break
  fi
  sleep 1
...
if [ "$READY" = false ]; then
  echo "ERROR: kolargrep serve failed to start within 60s" >&2
  exit 1
fi
...
echo "==> Memory before kolargrep benchmark:"
log_mem

echo "==> Benchmarking kolargrep (client -> serve)..."
KOLARGREP_START=$(now_ns)
QIDX=0
for pattern in "${QUERIES[@]}"; do
  QIDX=$((QIDX + 1))
  echo "  [$QIDX/$QUERY_COUNT] $pattern"
  "$KOLARGREP_BIN" "$pattern" "$BENCH_REPO_DIR" --index-path "$INDEX_PATH" > /dev/null 2>&1 || true
done
KOLARGREP_END=$(now_ns)
KOLARGREP_MS=$(( (KOLARGREP_END - KOLARGREP_START) / 1000000 ))
echo "kolargrep: ${KOLARGREP_MS}ms total"

echo "==> Memory after kolargrep benchmark:"
log_mem

# ── Stop serve ──
echo "==> Stopping kolargrep serve..."
trap - EXIT
cleanup_serve
...
# ── Write results ──
KOLARGREP_AVG=$(awk "BEGIN { printf \"%.1f\", $KOLARGREP_MS / $QUERY_COUNT }")
...
- **kolargrep mode**: client/server — \`kolargrep serve\` runs in background, \`kolargrep\` client connects via TCP
...
echo "| kolargrep (client → serve) | $KOLARGREP_MS | $KOLARGREP_AVG | - |" >> "$RESULTS_PATH"
