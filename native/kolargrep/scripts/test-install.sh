...
archive="kolargrep-v0.0.0-aarch64-apple-darwin.tar.gz"
printf '#!%s\nprintf "kolargrep 0.0.0\\n"\n' "$bash_bin" > "$tmpdir/fixture/kolargrep"
tar czf "$tmpdir/fixture/$archive" -C "$tmpdir/fixture" kolargrep
...
run_case() (
    ...
    PATH="$case_dir/bin:$tmpdir/tools" \
        ...
        KOLARGREP_VERSION=v0.0.0 \
        KOLARGREP_INSTALL_DIR="$case_dir/install" \
    ...
    if "$expect_success"; then
        if [ "$status" -ne 0 ] || [ ! -x "$case_dir/install/kolargrep" ]; then
            cat "$case_dir/output"
            fail "$tools/$scenario: installation did not succeed"
        fi
        cmp "$tmpdir/fixture/kolargrep" "$case_dir/install/kolargrep"
    else
        [ "$status" -ne 0 ] || fail "$tools/$scenario: installer should fail"
        [ ! -e "$case_dir/install/kolargrep" ] || fail "$tools/$scenario: installed an unverified binary"
