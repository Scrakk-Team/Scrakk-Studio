# kolargrep for coding agents

A short guide for AI agents (and the humans who wire them up) that want to
use `kolargrep` as a fast search tool inside a repository. It complements the
[README](README.md), which documents every flag; this file covers the few
things an agent has to get right.

## The mental model

kolargrep is ripgrep with a pre-built trigram index and an optional server.

```
kolargrep index .        # once: build the index into ./.kolargrep
kolargrep serve .        # once per session: keep the index warm and watch for changes
kolargrep "pattern" .    # every search: finds the server, answers in milliseconds
```
...
2. **On-disk index** but no server: read `.kolargrep/` directly. Fast, but only as
   ...
   Rebuild with `kolargrep index .` or resume `kolargrep serve .` to enable indexed queries.
3. **No index**: scan every file, like grep. Correct but slow on large trees.
   kolargrep prints a warning on stderr when this happens.
...
`kolargrep status .` shows `Hidden coverage: complete` once indexed queries are
...
```bash
brew install kolargrep                      # macOS, Linux
cargo install --path kolargrep-cli --locked # from a checkout
```
...
It writes `.kolargrep/serve.json` (PID and port) so clients can find it. Do not
commit `.kolargrep/`; add it to `.gitignore`.
...
and run `kolargrep index .` instead. Searches then use the on-disk index. That
index is not updated by searches or edits, so re-run `kolargrep index .` after any
...
```bash
kolargrep -- "fn parse_config" .                 # regex, default
kolargrep -F -- "Vec<Option<T>>" .               # literal string
kolargrep -w -t rust -- handle .                 # whole word, Rust files only
kolargrep -g "src/**" -C 2 -- "TODO|FIXME" .     # glob scope, 2 lines of context
kolargrep -l -- "impl .* for Server" .           # file names only
kolargrep -c -- deprecated .                     # count per file
kolargrep --files -t py .                        # list searchable Python files
```
...
- **Put `--` before the pattern** and pass the search root explicitly. Shell
  ...
  `kolargrep -- serve .` searches for the word. Everything after `--` is read as
...
```bash
kolargrep --json -F -- "fn main" kolargrep-cli/build.rs
```

```json
{"data":{"path":{"text":"kolargrep-cli/build.rs"}},"type":"begin"}
{"data":{"absolute_offset":400,"line_number":11,"lines":{"text":"fn main() {\n"},"path":{"text":"kolargrep-cli/build.rs"},"submatches":[{"end":7,"match":{"text":"fn main"},"start":0}]},"type":"match"}
{"data":{"binary_offset":null,"path":{"text":"kolargrep-cli/build.rs"},"stats":{"bytes_printed":260,"bytes_searched":1775,"elapsed":{"human":"0.000010s","nanos":9709,"secs":0},"matched_lines":1,"matches":1,"searches":1,"searches_with_match":1}},"type":"end"}
...
that is not valid UTF-8, ripgrep emits base64 `lines.bytes`, while kolargrep
...
reconciling the tree; `kolargrep index .` can rebuild it explicitly.
...
- With only an **on-disk index**, results reflect the last `kolargrep index`.
  Files created since then are not found. Run `kolargrep index .` again, or
  start `kolargrep serve .`.
- `kolargrep --files` reads from the index too. Add `--no-index` to list what is
  on disk right now.
...
```bash
kolargrep index . --index-path /tmp/idx --exclude vendor
kolargrep serve . --index-path /tmp/idx --exclude vendor
kolargrep "pattern" . --index-path /tmp/idx
```

## Repositories without `.git`

kolargrep indexes plain directories normally, but like ripgrep it ignores
`.gitignore` files outside a Git repository. The index is then larger than
expected, and kolargrep prints a warning saying so. Pass `--no-require-git` to
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `warning: no index at ... - scanning every file` | No index at the path the search looked in | If a server or index uses `--index-path`, pass the same value to the search; otherwise run `kolargrep index .` or `kolargrep serve .` |
| `Server unreachable, falling back to local index` | Server died or `serve.json` is stale | Restart `kolargrep serve .` |
| A new file is not found, no server | On-disk index predates the file | Run `kolargrep index .` |
| A new file is not found, server running | First build still in progress, or the watcher event is still queued | Wait, or pass `--no-index` for this search; re-running `kolargrep index .` does not update a running server |
| Search is slow despite a server | Flag bypasses the index (see above) | Drop the flag or scope with `-g`/`-t` |

## Tool definition sketch

If you expose kolargrep to a model as a tool, a minimal schema is:
...
Before invoking kolargrep, canonicalize `path` beneath the configured repository root
and reject paths that resolve outside it. Allowlist search-only flags rather than
forwarding arbitrary tokens. Then run `kolargrep <flags...> -- <pattern> <path>` and
