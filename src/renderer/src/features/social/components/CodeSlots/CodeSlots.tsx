/**
 * CodeSlots — input de código OTP con slots animados.
 *
 * Port a TSX (mismo comportamiento) del componente CodeSlots de React Bits
 * (variante JavaScript + CSS). Dependencias: `motion`, `@hugeicons/react` y
 * `@hugeicons/core-free-icons`.
 *
 * Lo usa el flujo de verificación por email del panel Social.
 */
...
export interface CodeSlotsProps {
  /** Número de slots. */
  length?: number
  /** Código controlado. Los dígitos que llegan de afuera aterrizan en cascada. */
  value?: string
  /** Código inicial (no controlado), ya aterrizado. */
  defaultValue?: string
  /** Se llama en cada edición (incluido el clear al final de un rechazo). */
  onChange?: (code: string) => void
  /** Se llama una vez cuando se llena el último slot. */
  onComplete?: (code: string) => void
  /** error drena los slots y limpia; success funde los fills y bloquea. */
  status?: CodeSlotsStatus
  /** Muestra un punto en lugar del dígito. */
  mask?: boolean
  /** Caret parpadeante en el slot activo. */
  caret?: boolean
  disabled?: boolean
  autoFocus?: boolean
  accentColor?: string
...
export default function CodeSlots({
  ...
  const statusRef = useRef<CodeSlotsStatus>(status)
  ...
  const springs = useMemo(
    () => ({
      mvs: Array.from({ length }, (_, i) => motionValue<number>(slotsRef.current[i] ? 1 : 0)),
      drops: Array.from({ length }, () =>
        motionValue<number>(statusRef.current === 'success' ? 1 : 0)
      )
    }),
    [length]
  )
  ...
  const drive = useCallback(
    (i: number, to: number, delayMs = 0): void => {
      const mv = mvs[i]
      if (!mv) return
      target.current[i] = to
  ...
  const moveActive = useCallback(
    (next: number, crossed: number[]): void => {
  ...
  const caretX = useTransform((): number => {
  ...
  const caretTransform = useTransform(caretX, (x: number): string => `translateX(${x}px)`)
  const washClip = useTransform(
    openMv,
    (o: number): string => `inset(0 ${(1 - clamp01(o)) * 50}% round ${washRadius}px)`
  )
  const checkTransform = useTransform(
    checkMv,
    (c: number): string => `translateY(${(1 - c) * CHECK_RISE}px) scale(${0.85 + 0.15 * Math.max(c, 0)})`
  )
  ...
  const insert = (raw: string, from: number = active): void => {
  ...
  const clearSlot = (i: number, stepBack = false): void => {
  ...
  const onRowMouseDown = (e: MouseEvent<HTMLDivElement>): void => {
    if (disabled) return
    e.preventDefault()
  ...
  return (
    ...
    >
      ...
      <span id={`${uid}-count`} className="code-slots__sr" aria-live="polite">
        {status === 'success'
          ? 'Código aceptado'
          : `${view.filter(Boolean).length} de ${length} dígitos ingresados`}
      </span>
    </div>
  )
}
...
function Slot({
  ...
  const fill = useTransform(mv, (t: number): string => `scale(${Math.max(t, 0)})`)
  const lift = useTransform(
    [mv, drop],
    ([t, d]: number[]): string => `translateY(${(1 - t) * rise + Math.max(d, 0) * sink}px)`
  )
  const ink = useTransform(
    [mv, drop],
    ([t, d]: number[]): number => clamp01(t) * (1 - clamp01(d / SINK_FADE))
  )
  return (
    <span
